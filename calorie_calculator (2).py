...
def analyze_food_image(image_bytes: bytes, api_key: str) -> dict:
    """
    ส่งรูปภาพอาหารให้ Groq (Llama Vision) วิเคราะห์แคลอรีและ Macronutrients

    วิธีทำงาน:
    1. แปลงรูปภาพเป็น base64 string
    2. สร้าง Groq client และส่ง request พร้อมรูปและ prompt
    3. Parse JSON ที่ AI ตอบกลับมาเป็น Python dict

    Parameters:
        image_bytes : ข้อมูลรูปภาพในรูปแบบ bytes (จาก st.file_uploader)
        api_key     : Groq API Key (ฟรี! จาก console.groq.com)
    ...
    # ── Step 2: สร้าง Groq client ──
    client = Groq(api_key=api_key)

    # ── Step 3: เรียก Groq API พร้อมส่งรูป ──
    # ใช้ llama-4-scout ที่รองรับ Vision (มองเห็นรูปภาพได้)
        ...
        messages=[
            {
                ...
                "content": [
                    ...
                    {
                        "type": "text",
                        "text": """วิเคราะห์อาหารในรูปนี้ แล้วตอบเป็น JSON เท่านั้น ห้ามมีข้อความอื่น ห้ามมี ```json
{
  ...
  "notes": "หมายเหตุสั้นๆ"
    ...
    # ── Step 4: แปลง response เป็น Python dict ──
...
with tab_food:
    ...
    st.write("อัปโหลดรูปอาหาร แล้วให้ Groq AI (Llama Vision) บอกชื่ออาหาร แคลอรี และ Macronutrients ให้เลย! (ฟรี 100% 🎉)")

    # ── กรอก API Key ──
    # st.text_input + type="password" ทำให้ข้อความถูกซ่อน (แสดงเป็น ***)
    api_key = st.text_input(
        "🔑 Groq API Key (ฟรี 100%!)",
        type="password",
        placeholder="gsk_...",
        help="รับ API Key ฟรีได้ที่ https://console.groq.com",
    )

    # แสดงลิงก์ขอ API Key ถ้ายังไม่มี
    if not api_key:
        st.info("💡 ยังไม่มี API Key? รับฟรีได้ที่ [console.groq.com](https://console.groq.com) → Sign up → API Keys → Create API Key")
    ...
    with result_col:
        ...
        if uploaded_file is not None and analyze_btn and api_key:
            ...
            with st.spinner("🤖 Groq AI กำลังวิเคราะห์อาหาร..."): 
                try:
                    # อ่านไฟล์เป็น bytes
...
<div style="text-align:center; color:#aaa; font-size:0.8rem; padding:16px 0">
    🔬 ใช้สมการ <b>Harris-Benedict (Revised)</b> | Macro อ้างอิง <b>AMDR</b> | AI วิเคราะห์ภาพด้วย <b>Groq Llama Vision API</b><br>
